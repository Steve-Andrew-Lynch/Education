...
def make_str_from_row(board, row_index):
    ...
    result = str("".join(board[row_index]))
    return result
...
def board_contains_word_in_column(board, word):
    ...
    False
    """
    for column_index in range((len(board[0]) - 1)):
...
def read_board(board_file):
    ...
    """
    board = []
    letters = [] ##Letters to save your place in each list
    brd_file = board_file
    line = brd_file.readline()
    while line != '':
        if line != '\n':
            letters = []
            for l in line:
