...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    #a running list to track the letter values, later summed
    aResult = list(word)
    #the score
    aScore = 0
    #length of word
    ...
    if len(word) == 0:
        aScore = 0
        return aScore
    #find letter scorein dictionary SCRABBLE_LETTER_VALUES and add to list aResult
    if len(word) > 0:
        for i in range(len(aResult)):
            aResult[i] = SCRABBLE_LETTER_VALUES[aResult[i]]
        #sum the list aResult, multiply that by len
        aScore = sum(aResult)
        aScore = aScore * len(word)
        if n == len(word):
            aScore = aScore + 50
        return aScore
...
def isValidWord(word, hand, wordList):
    ...
    for i in word:
        if i not in theHand:
            ...
            theHand[i] -= 1
    isWordTrue = True
    #if so, does it pass the other tests?
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    # As long as there are still letters left in the hand:
        # if gameRunning: -may need to come back to this due to while loop issues
    while calculateHandlen(hand) > 0:
        ...
        # Ask user for input
        anInput = raw_input('Enter a word: ')
        anInput == str(anInput)
        word = anInput
